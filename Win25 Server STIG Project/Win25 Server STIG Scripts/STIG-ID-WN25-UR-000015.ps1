<#
.SYNOPSIS
    Ensures the Guest account is denied the "Log on as a batch job" right on Windows Server 2025.
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\STIG-ID-WN25-UR-000015.ps1
#>
...
if ($content -match "SeDenyBatchLogonRight\s*=\s*(.*)") {
    ...
    if ($existing -notmatch [regex]::Escape($guestsSid)) {
        ...
        $content = $content -replace "SeDenyBatchLogonRight\s*=.*", "SeDenyBatchLogonRight = $newVal"
    }
} else {
    $content += "`r`nSeDenyBatchLogonRight = $guestsSid"
}
